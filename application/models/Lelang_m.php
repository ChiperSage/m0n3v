...
class Lelang_m extends CI_Model {
	...
	{
		$tahun = date('Y');
		$key = array('tahun'=>$tahun);

		if(isset($_GET['tahun']) && $_GET['tahun'] != ''){
			$key = array('tahun'=>$_GET['tahun']);
			$tahun = $_GET['tahun'];
		}

		$str = "SELECT l.*, r.nama_satker FROM tb_lelang l
		LEFT JOIN tb_rup r ON l.kode_rup = r.kode_rup
		WHERE l.tahun LIKE '%$tahun%' GROUP BY l.kode_lelang";
	...
	public function get_lelang_spse()
	{
		$tahun = date('Y');
		if(isset($_GET['tahun']) && $_GET['tahun'] != ''){
			$tahun = $_GET['tahun'];
		...
		$this->db->select('*');
		$this->db->from('tb_lelang_spse');
		$this->db->where(array('ang_tahun'=>$tahun));
		return $this->db->get()->result();
	}
